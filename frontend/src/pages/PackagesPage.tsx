    ...
    <SiteLayout>
      ...
      <section className="mx-auto max-w-6xl px-4 pb-20 sm:px-6">
        <div className="grid gap-6 md:grid-cols-3">
          {PACKAGES.map((p) => (
            <div key={p.id} className={"relative flex flex-col overflow-hidden rounded-2xl border p-6 backdrop-blur transition-all " +
              (p.highlight ? "border-primary/50 bg-card/80 shadow-glow" : "border-white/10 bg-card/60 hover:border-primary/30")}>
              {p.tag && (
              ...
              <ul className="space-y-2.5">
                {p.features.map((f) => (
                  <li key={f} className="flex items-start gap-2 text-sm">
                    <CheckCircle2 className="mt-0.5 h-4 w-4 text-accent" /><span>{f}</span>
                  </li>
                ))}
              </ul>
